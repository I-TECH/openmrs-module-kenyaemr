...
public class EligibleForArtTriggerCalculation extends BaseEmrCalculation {
	...
	private List<Obs> extractTriggers(List<Obs> obss) {
		...
		for (Obs o : obss) {
			...
			if (o.getConcept().equals(getConcept(Dictionary.DATE_OF_HIV_DIAGNOSIS))) {
			...
			else if (o.getConcept().equals(getConcept(Dictionary.CURRENT_WHO_STAGE))) {
			...
			else if (o.getConcept().equals(getConcept(Dictionary.CD4_COUNT))) {
			...
			else if (o.getConcept().equals(getConcept(Dictionary.CD4_PERCENT))) {
	...
	private Obs findEarliestTrigger(List<Obs> triggers) {
		...
		for (Obs trigger : triggers) {
			...
			Date triggerDate = trigger.getConcept().equals(getConcept(Dictionary.DATE_OF_HIV_DIAGNOSIS))
