...
// look all active in care who were presumed for tb
        String sqlQuery = "select  e.patient_id  \n" +
                ...
                "           (((date(latest_tca) between date(:startDate) and date(:endDate)) and ((date(latest_vis_date) >= date(latest_tca)) or date(latest_tca) > curdate()) ) or ((timestampdiff(DAY,date(latest_tca),date(:endDate)) <= 30 or timestampdiff(DAY,date(latest_tca),date(curdate())) <= 30))\n" +
                "               and (date(latest_tca) >= date(date_discontinued) or disc_patient is null )) and (screened_using_icf =142177 or (screened_using_consultation=142177 and person_present=978)))\n" +
                "           ) e;";
