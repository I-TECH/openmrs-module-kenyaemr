...
public class EligibleForArtTriggerCalculation extends BaseEmrCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		CalculationResultMap confirmedPositives = allObs(getConcept(MetadataConstants.DATE_OF_HIV_DIAGNOSIS_CONCEPT_UUID), cohort, context);
		CalculationResultMap whoStages = allObs(getConcept(MetadataConstants.CURRENT_WHO_STAGE_CONCEPT_UUID), cohort, context);
		CalculationResultMap cdCounts = allObs(getConcept(MetadataConstants.CD4_CONCEPT_UUID), cohort, context);
		CalculationResultMap cdPercents = allObs(getConcept(MetadataConstants.CD4_PERCENT_CONCEPT_UUID), cohort, context);
	...
	private List<Obs> extractTriggers(List<Obs> obss) {
		...
		for (Obs o : obss) {
			...
			if (o.getConcept().getUuid().equals(MetadataConstants.DATE_OF_HIV_DIAGNOSIS_CONCEPT_UUID)) {
			...
			else if (o.getConcept().getUuid().equals(MetadataConstants.CURRENT_WHO_STAGE_CONCEPT_UUID)) {
				if (KenyaEmrUtils.whoStage(o.getValueCoded()) >= 3) {
					isTrigger = true;
				}
			}
			// Did patient's CD4 count drop below: 1000 (25-59 months) or 500 (5-12 years) or 350 (everyone else)
			else if (o.getConcept().getUuid().equals(MetadataConstants.CD4_CONCEPT_UUID)) {
			...
			else if (o.getConcept().getUuid().equals(MetadataConstants.CD4_PERCENT_CONCEPT_UUID)) {
	...
	private Obs findEarliestTrigger(List<Obs> triggers) {
		...
		for (Obs trigger : triggers) {
			...
			Date triggerDate = trigger.getConcept().getUuid().equals(MetadataConstants.DATE_OF_HIV_DIAGNOSIS_CONCEPT_UUID)
