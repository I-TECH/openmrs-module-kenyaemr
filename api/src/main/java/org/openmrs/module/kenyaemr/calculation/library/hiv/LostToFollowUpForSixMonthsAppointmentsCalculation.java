 ...
 */
public class LostToFollowUpForSixMonthsAppointmentsCalculation extends AbstractPatientCalculation implements PatientFlagCalculation {
		...
		for (Integer ptId : cohort) {
			...
			if (inHivProgram.contains(ptId)) {
				...
				if (lastScheduledReturnDate != null) {
					if(daysSince(lastScheduledReturnDate, context) > HivConstants.LOST_TO_FOLLOW_UP_6_MONTHS_APPOINTMENT_THRESHOLD_DAYS){
						lost = true;
					}
