...
public class TXCurrLinelistCohortLibrary {
    ...
    public  CohortDefinition txCurLinelistForPatientsPresentInCurrentButMissingInPreviousReportMonthly() {
        String sqlQuery="select cr.patient_id\n" +
                ...
                "  join kenyaemr_etl.etl_patient_demographics p on p.patient_id=fup.patient_id\n" +
                "  join kenyaemr_etl.etl_hiv_enrollment e on fup.patient_id=e.patient_id\n" +
                "  left outer join kenyaemr_etl.etl_drug_event de on e.patient_id = de.patient_id and date(date_started) <= date_sub(DATE(:endDate), interval 1 MONTH)\n" +
                "  left outer JOIN\n" +
                "  (select patient_id, visit_date from kenyaemr_etl.etl_patient_program_discontinuation\n" +
                "  where date(visit_date) <= date_sub(DATE(:endDate), interval 1 MONTH) and program_name='HIV'\n" +
                "  group by patient_id\n" +
                "  ) d on d.patient_id = fup.patient_id\n" +
                "  where fup.visit_date <= date_sub(DATE(:endDate), interval 1 MONTH)\n" +
                "  group by patient_id\n" +
                "  having (started_on_drugs is not null and started_on_drugs <> \"\") and ( \n" +
                "  ( (disc_patient is null and date_add(date(latest_tca), interval 30 DAY)  >= date_sub(DATE(:endDate), interval 1 MONTH)) or (date(latest_tca) > date(date_discontinued) and date(latest_vis_date)> date(date_discontinued) and date_add(date(latest_tca), interval 30 DAY)  >= date_sub(DATE(:endDate), interval 1 MONTH) ))\n" +
                "  )\n" +
                ") pr on pr.patient_id=cr.patient_id\n" +
                "where pr.patient_id is null;";
        SqlCohortDefinition cd = new SqlCohortDefinition();
