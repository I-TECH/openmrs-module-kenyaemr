...
public class WHOStagesAtEnrollmentsCalculationTest extends BaseModuleContextSensitiveTest {
	...
	public void evaluate_shouldCalculateWHOStagesAtEnrollments() throws Exception {
		...
		// Give patient #6 a WHO PEDS 1 STAGE obs on 2010-1-1
		Concept whoPeds1 = Context.getConceptService().getConceptByUuid(MetadataConstants.WHO_STAGE_1_PEDS_CONCEPT_UUID);
		TestUtils.saveObs(ps.getPatient(6), whoStage, whoPeds1, TestUtils.date(2010, 1, 1));

		// Enroll patient #6 HIV Program on 2011-1-1 until 2011-2-1
		PatientProgram patient6Enrollment1 = TestUtils.enrollInProgram(ps.getPatient(6), hivProgram, TestUtils.date(2011, 1, 1), TestUtils.date(2011, 2, 1));
		...
		PatientProgram patient6Enrollment2 = TestUtils.enrollInProgram(ps.getPatient(6), hivProgram, TestUtils.date(2011, 3, 1));
		...
		Assert.assertEquals(whoAdult1, patient6Results.get(patient6Enrollment1));
		Assert.assertEquals(whoAdult3, patient6Results.get(patient6Enrollment2));
