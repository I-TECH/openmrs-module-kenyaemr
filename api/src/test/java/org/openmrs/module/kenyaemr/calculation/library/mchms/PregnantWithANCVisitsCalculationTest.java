...
public class PregnantWithANCVisitsCalculationTest extends BaseModuleContextSensitiveTest {
    ...
    public void evaluate_shouldCalculateWomenWithANCVisits() throws Exception {
        ...
        Patient patient2 = TestUtils.getPatient(8);*/



        //enroll patient #2 to mchms program
        ...
        //Ennroll Pat#7 into MCH and create an ANC encounter with WHO staging
        {
            ...
            Obs[] enrollmentEncounterObs = {TestUtils.saveObs(patient, hivStatusConcept, Dictionary.getConcept(Dictionary.POSITIVE), TestUtils.date(2014, 1, 20))};
            TestUtils.saveEncounter(patient, ancEncounterType, ancForm, TestUtils.date(2014, 1, 20), enrollmentEncounterObs);
            Obs[] whoStageObs = {TestUtils.saveObs(patient, whoStageConcept, Dictionary.getConcept(Dictionary.WHO_STAGE_1_ADULT), TestUtils.date(2014, 1, 20))};
            TestUtils.saveEncounter(patient, ancEncounterType, ancForm, TestUtils.date(2014, 1, 20), whoStageObs);
        }
        ...
        //add encounters anc form to patient #7
        /*TestUtils.saveEncounter(patient, encounterType, ancForm, TestUtils.date(2014, 1, 20));
        TestUtils.saveEncounter(patient, encounterType, ancForm, TestUtils.date(2014, 2, 20));
        TestUtils.saveEncounter(patient, encounterType, ancForm, TestUtils.date(2014, 3, 20));
        TestUtils.saveEncounter(patient, encounterType, ancForm, TestUtils.date(2014, 4, 20));
        TestUtils.saveEncounter(patient, encounterType, ancForm, TestUtils.date(2014, 5, 20));

        //adding three anc encounters for patient #8
        TestUtils.saveEncounter(patient2, encounterType, ancForm, TestUtils.date(2014, 1, 20));
        TestUtils.saveEncounter(patient2, encounterType, ancForm, TestUtils.date(2014, 2, 20));
        TestUtils.saveEncounter(patient2, encounterType, ancForm, TestUtils.date(2014, 4, 20));*/

        //add pregnancy obs for patient#7

        /*TestUtils.saveObs(patient2, pregnancyStatus, yes, TestUtils.date(2014, 9, 10));
        TestUtils.saveObs(patient2, pregnancyStatus, yes, TestUtils.date(2014, 8, 10));
        TestUtils.saveObs(patient2, pregnancyStatus, yes, TestUtils.date(2014, 7, 10));*/
